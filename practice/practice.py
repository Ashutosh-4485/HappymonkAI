...
VIDEOS_DIR = "D:\Coding files\Python\pythonProject\happymonk"
video_path = os.path.join(VIDEOS_DIR, '1.mp4')
video_path_out = '{}_out.webm'.format(video_path)

cap = cv2.VideoCapture("D:\Coding files\Python\pythonProject\happymonk\1.mp4")
...
while ret:
    ...
    for res in results:
        filtered_indeces = \
        np.where((np.isin(res.boxes.cls.cpu().numpy(), [2, 3, 5, 7])) & (res.boxes.conf.cpu().numpy() > 0.05))[0]
        ...
        for xmin, ymin, xmax, ymax, track_id in tracks:
            ...
            cross_line_right = (PASS_LINE_RIGHT[1][0] - PASS_LINE_RIGHT[0][0]) * (yc - PASS_LINE_RIGHT[0][1]) - (
                        PASS_LINE_RIGHT[1][1] - PASS_LINE_RIGHT[0][1]) * (xc - PASS_LINE_RIGHT[0][0])
    ...
    for res in results_left:
        ...
        for xmin, ymin, xmax, ymax, track_id in tracks_left:
            ...
            if track_id not in cross_orange_line:
                cross_orange = (ORANGE_LINE[1][0] - ORANGE_LINE[0][0]) * (yc - ORANGE_LINE[0][1]) - (
                ...
                cross_yellow = (YELLOW_LINE[1][0] - YELLOW_LINE[0][0]) * (yc - YELLOW_LINE[0][1]) - (
                ...
                cross_pink = (PINK_LINE[1][0] - PINK_LINE[0][0]) * (yc - PINK_LINE[0][1]) - (
            ...
            cross_line_left = (PREV_LINE_LEFT[1][0] - PREV_LINE_LEFT[0][0]) * (yc - PREV_LINE_LEFT[0][1]) - (
                        PREV_LINE_LEFT[1][1] - PREV_LINE_LEFT[0][1]) * (xc - PREV_LINE_LEFT[0][0])
